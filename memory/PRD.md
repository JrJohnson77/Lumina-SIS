...
- **Jun 2026 (Session 20 — Object Storage + Audit completion)**:
  ...
  - **Broken images fixed**: Wrapped every remaining `/api/uploads` `<img>` source with `mediaUrl()` token helper across StudentsPage, UsersPage, StaffProfilePage, DashboardTab, ReportsPage, ImportExportPage, SchoolsPage, MHPSReportCardTemplate, LuminaDefaultReportCard.
  - **Testing**: Backend 22/22 (100%) — see `/app/test_reports/iteration_19.json` and `/app/backend/tests/test_object_storage_and_audit.py`. Frontend smoke verified (login + Users page render, logo loads).
  - **Still pending (P1)**: Task 2.3 pagination (`skip`/`limit` on list endpoints + frontend hooks) — NOT STARTED.

- **Feb 2026 (Session 19 — Lumina Default Report Card)**: Built a production-ready, distinctive legal-size (8.5×14, single page) default report card auto-available to every school until they customize.
  - **Component**: `/app/frontend/src/components/LuminaDefaultReportCard.jsx` — indigo accent (pulls from `school.primary_color`), letter-grade colored badges (A→emerald, B→blue, C→amber, D→orange, F/E→red), horizontal attendance tile strip, 5-dot conduct scale (Punctuality/Participation/Behaviour/Effort), promotion pill (Promoted / On Probation / Retained based on overall score), homeroom-teacher + principal comment boxes, stamp/seal area, parent signature line, and 'Generated by Lumina-SIS' watermark in the footer.
  - **Dispatch**: `ReportCardRenderer` in `ReportsPage.js` returns `LuminaDefaultReportCard` when `template.design_mode === 'lumina_default'` (the new default) or the template is empty; canvas-customized templates still take precedence when canvas_elements are present.
  - **Backend**: `build_default_template` now stamps `design_mode='lumina_default'` + `is_locked_default=true`; new `POST /api/report-templates/{school_code}/reset-default` (admin/superuser) wipes canvas_elements and writes a `reset` audit row.
  - **Designer**: Added a Sparkles 'Lumina default' button in the toolbar that confirms then resets canvas customizations.
  - **PDF filename**: Single-student term-report exports now use the spec format `[LastName]_[FirstName]_ReportCard_[Term]_[AcademicYear].pdf`; class batch exports keep the legacy `[ClassName]_[Term]_[Year].pdf`.
  - **Access**: `/report-template` route + sidebar link now open to both `admin` and `superuser`; the designer defaults the school to the current user's own `school_code` when no `?school=` query param is provided.
  - **Testing**: Backend 7/7 (100%) — see `/app/test_reports/iteration_14.json` and `/app/backend/tests/test_lumina_default.py`. Frontend ~95% (watermark count = 11 cards, badges + conduct dots + attendance tiles all rendered, reset-default flow works end-to-end). One route-permission gap fixed post-test.
...
- **Feb 2026 (Session 17 — 20-item enhancement batch)**:
  ...
  - **Attendance**: Defaults every student to "Present" so teachers only mark exceptions (P/A/L/E); monthly summary endpoint `GET /api/students/{id}/attendance/summary?month=YYYY-MM` returns counts + percent_present + threshold + below_threshold flag; school's `attendance_threshold` (default 85%) configurable.
  - **Students**: New `/students/:studentId` profile page with 6 tabs (Overview/Grades/Attendance/Health/Discipline/Documents) pulling live API data; sibling auto-detection on matching guardian email; clickable student names link to profile; photo_url already supported.
  - **Report cards**: PDF-quality preview modal via `html2canvas + jspdf` (in-browser iframe); Send-to-parents button (console.log + toast in dev); admin-only Lock-all / Unlock-all toggle with LOCKED badge on locked cards; new `POST/DELETE /api/report-cards/{id}/lock` + `GET /api/report-cards/locks`.
  - **Admissions**: Inquiry→Application→Enrolled pipeline complete — `POST /api/admissions/{id}/convert` creates the Student row with `enrollment_status='enrolled'` and stores `converted_student_id` (idempotent); UI shows "Convert to student" only on accepted/unconverted, "Enrolled" badge once linked.
  - **Data Integrity**: Shared `<ConfirmDialog />` replaces every `window.confirm` (Students, Classes, Schools, Grades, Admissions) — always shows the record name + destructive styling; CSV import shows per-row validation report (red highlight + Issues column) before submission; new admin-only `/audit-logs` page (`GET /api/audit-logs`) reads from `audit_logs` collection populated by `write_audit()` helper hooked into create/update/delete/lock/convert ops.
  - **Testing**: Backend 27/27 (100%) on first pass + regressions on second pass — see `/app/test_reports/iteration_13.json` and `/app/test_reports/iteration_14.json`. Frontend smoke + targeted e2e all green.
